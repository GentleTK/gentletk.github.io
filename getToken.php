...
$check_url = "https://ssl.ptlogin2.qq.com/check?regmaster=&pt_tea=2pt_vcode=1&uin=$uin&appid=$appid";
...
$status = $checkVCSub3[0];                                  // 0:表示参数2为验证码  1:需要验证
$verifycode = $checkVCSub3[1];                              // 验证码
//$cap_cd = $checkVCSub3[1];                                  // cap_cd
$salt = $checkVCSub3[2];                                    // 加密需要
$pt_verifysession_v1 = $checkVCSub3[3];                     // 验证码句柄

//$get_vsig_url = "http://captcha.qq.com/cap_union_new_show?aid=$appid&asig=&captype=&protocol=http&clientype=2&disturblevel=&apptype=2&curenv=inner&noBorder=noborder&showtype=embed&uid=$uin&cap_cd=$cap_cd&lang=2052";
//$vsig_resp = getResponse($get_vsig_url);
...
function entry() {
    ...
    var pswd = $.Encryption.getEncryption("<?php echo $password;?>", salt, "<?php echo $verifycode;?>", "");
...
// post请求
function getCookieResponse($url)
{
    // 初始化curl //转义
    $ch = curl_init();            
    // 抓取指定网页 
    ...
    curl_setopt($ch, CURLOPT_SSL_VERIFYHOST, false);
    // 设置cookie保存到文件
    curl_setopt($ch, CURLOPT_COOKIEJAR, 'pstcookie.txt');
    //运行curl
    $data = curl_exec($ch); 
    curl_close($ch);
    return $data;
...
echo "<script>document.write(entryResult)</script>";
$login_p = $_COOKIE['pswd'];
echo "<br>login_p:$login_p";


//$login_pswd = file_get_contents("login_pswd.txt");
//echo "<br>login_p:$login_pswd";

//$login_pswd = "<script>document.writeln(entryResult)</script>";

//$login_pswd_sub1 = substr($login_pswd, 46, 10);
//var_dump($login_pswd_sub1);
//echo $login_pswd_sub1;
//$login_p = $login_pswd;
...
$login_url = "https://ssl.ptlogin2.qq.com/login?u=$uin&verifycode=$verifycode&pt_vcode_v1=0&pt_verifysession_v1=$pt_verifysession_v1&p=$login_p&pt_randsalt=2&u1=https://im.qq.com/loginSuccess&ptredirect=0&h=1&t=1&g=1&from_ui=1&ptlang=2052&action=$action&js_ver=$js_ver&js_type=$js_type&login_sig=&pt_uistyle=40&aid=$appid&ptdrvs=$ptdrvs";
$login_post_data = "u=$uin&verifycode=$verifycode&pt_vcode_v1=0&pt_verifysession_v1=$pt_verifysession_v1&p=$login_p&pt_randsalt=2&u1=https://im.qq.com/loginSuccess&ptredirect=0&h=1&t=1&g=1&from_ui=1&ptlang=2052&action=$action&js_ver=$js_ver&js_type=$js_type&login_sig=&pt_uistyle=40&aid=$appid&ptdrvs=$ptdrvs";

echo "<br>login_url: $login_url"."<br>";
//echo "<br><a href='$login_url' target='_Blank'>$login_url</a>";

$getCookies = httpRequest($login_base_url, $login_post_data);
